...
private:
    ...
    struct Vertex {
        glm::vec2 pos;
        glm::vec3 color; // not actually using vertex color right now but want to keep it around just in case

        // "how to pass this data fmt to the vert shader 
        //  once it's been uploaded into GPU memory"
        static vk::VertexInputBindingDescription getBindingDescription() {
            return {
                0,                            // idx of binding in the array of bindings
                sizeof(Vertex),               // stride
                vk::VertexInputRate::eVertex  // eInstance is for instanced rendering, eVertex is for per-vertex data
            };
        };

        // "how to extract a vert attrib from a chunk of vert data 
        //  originating from a binding desc"
        // 1 attrib desc struct for each Vertex struct field (position, color) :)
        static std::array<
        ...
        > getAttributeDescriptions() {
            return {
                vk::VertexInputAttributeDescription(
                    0,                         // location?
                    0,                         // binding (idx)
                    vk::Format::eR32G32Sfloat, // signed float2
                    offsetof(Vertex, pos)),
                vk::VertexInputAttributeDescription(
                    1, 
                    0, 
                    vk::Format::eR32G32B32Sfloat, // signed float3
    ...
    void populateVertices(size_t res, float scale=1.f) {
        for (size_t i = 0; i <= res; i++) {
            for (size_t j = 0; j <= res; j++) {
                float x = 2. / (float)(res) * j - 1.;
                float y = 2. / (float)(res) * i - 1.;
                
                Vertex v {.pos = {x*scale, y*scale}, .color = {(float)i/res, (float)j/res, 1.}};
                vertices.push_back(v);
                if (i < res && j < res) {
                    indices.push_back((res+1)*i + j);
                    indices.push_back((res+1)*(i+1) + j);
                    indices.push_back((res+1)*(i+1) + j + 1);
                    indices.push_back((res+1)*(i+1) + j + 1);
                    indices.push_back((res+1)*i + j + 1);
                    indices.push_back((res+1)*i + j);
                }
            }
        }
        printf("Populated %lu vertices!\n", vertices.size());
    }
    ...
    void createGraphicsPipeline() {
        ...
        vk::PipelineVertexInputStateCreateInfo vertexInputInfo {
            ...
            .vertexAttributeDescriptionCount = attributeDescriptions.size(), // 2?
    ...
    void createIndexBuffer() {
        if (false) { // staging buffer version
    ...
    void createDescriptorSets() {
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            ...
            vk::WriteDescriptorSet descriptorWrite{
                .dstSet = descriptorSets[i], // desc set to update
                .dstBinding = 0,             // uniform buffer binding index
                .dstArrayElement = 0, // descriptors can be arrays but ours isn't
                .descriptorCount = 1, // number of arr elements to update
    ...
    void recordCommandBuffer(uint32_t imageIndex) {
        ...
        commandBuffers[currentFrame].bindIndexBuffer(
            *indexBuffer, 0, vk::IndexType::eUint32
        );
        commandBuffers[currentFrame].bindDescriptorSets(
            vk::PipelineBindPoint::eGraphics, // not unique to graphics pipeline
            pipelineLayout, 
            0, 
            *descriptorSets[currentFrame], 
            nullptr
        );
        // to be replaced with drawIndexed
    ...
    void updateUniformBuffer(uint32_t currentImage) {
        ...
        ubo.proj[1][1] *= -1; // invert y coord of clip coords
        // because glm was designed for opengl
        ubo.time = time;
